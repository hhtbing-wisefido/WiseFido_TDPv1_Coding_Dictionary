...
| 目录 | 用途 | 规则 |
|------|------|------|
| `coding_dictionary/` | 核心数据源 | ✅ 唯一可手动编辑的数据文件 |
| `auto_generated_docs/` | 自动生成的文档 | ⚠️ 禁止手动修改,所有自动生成的总结报告、文档都放这里 |
| `temp/` | 临时文件 | 🔧 所有临时文件、草稿、测试文件都放这里,定期清理 |
| `scripts/` | 工具脚本 | 📝 维护脚本,不放数据文件 |
| `schema/` | Schema 定义 | 📋 数据结构定义 |
| `spec/` | 规范文档 | 📖 数据规范说明 |
| `auto_backup/` | 自动备份 | 💾 脚本自动创建的备份 |
| 项目根目录 | 核心配置 | ⚠️ **仅放置 README.md、requirements.txt、.gitignore 等核心配置文件** |
