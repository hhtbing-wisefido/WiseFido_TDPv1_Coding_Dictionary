...
enum DangerLevel {
  UNKNOWN = 0;
  EMERGENCY = 1; //CoAP CON 紧急，如跌倒、心率 > 130(心脏功能可能已受损或存在严重应激) 或 < 40（导致脑供血不足，引起头晕、晕厥），持续 1 分钟; 呼吸率 > 28(严重的呼吸道感染、肺栓塞或心力衰竭的信号) 或 < 10（药物过量、脑部病变或严重的代谢紊乱），持续 1 分钟
  ALERT = 2; //CoAP CON 警报，高危事件，心率110-130，持续5分钟；呼吸率20-28，持续 5 分钟（身体正在努力应对某种应激，例如发热、轻微的肺炎或疼痛）
  CRITICAL = 3; //CoAP CON 严重，需关注，如心率持续90-110且持续5分钟（亚健康或病理状态的早期）;   呼吸率 10-12，持续 5 分钟(持续偏低或接近下限可能意味着嗜睡、镇静剂作用或呼吸驱动减弱，需要引起护理人员的警惕)
  ERROR = 4; // 错误，设备故障，如传感器断线
  WARNING = 5; //CoAP CON 警告，预警，如可疑跌倒、心率90-120
  NOTICE = 6; // CoAP CON 通知，下发配置指令
  INFORMATIONAL = 7; // 信息，设备上线
  DEBUG = 8; // 调试，未知或开发用途
  CANCEL = 9; //CoAP CON 表示取消警报
}
...
// L5=高风险,低置信：触发警示灯(30秒+120)计时器报警，确认方式=本地人工确认||Server计算.确认危险。 明确肯定：L1报警; 明确否认：CANCEL;未确认:计时器L2级报警
// L8=DEBUG: 由Server确认，内部记录，主要用于内部AI标识与训练
...
message LiteEventHeader {
  ...
  // 示例：Physiological.CardiacPause.Confirmed：确诊心率暂停 (>=3S)
}
...
**本地发出Warning警报时的Lite Event payload结构：**

```
repeated TextPayload text_payloads = 2;
```

**首次发出警报标识：** producer_id + 事件序列号作为全局警报标识

| 索引 | 字段 | 说明 |
|-----|------|------|
| [0] | `producer_id` | 警报初始时的producer_id，即第一个发出该警报的DeviceID |
| [1] | `sequence_number` | 初始producer生成时的序列号 |
| [2] | `semantic_location` | 事件位置，如"Room201" |
| [3] | `alert_timestamp` | 本条警报发送时间 |
| [4] | `countdown_seconds` | 倒计时器值 |
| [5] | `signature` | 使用CoAP PSK对前面0-4进行签名，仅SOS模式需要 |

**注：** Tag 定义见 FHIR与SNOMED_CT代码.md
